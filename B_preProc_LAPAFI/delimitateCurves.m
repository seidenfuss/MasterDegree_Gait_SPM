function [delimitated_data] = delimitateCurves(data,n_steps,b_R,x,y,z)
%DELIMITATECURVES: It creates i sub-groups containing k (N_steps) for each participant (i).
...
for i=1:dim(1,1)
    k=n_steps{i};
    for j=1:k
        delimitated_data{i,j,1} = data{i,2}(b_R{i,1}(1,j):b_R{i,2}(1,j),x);
        delimitated_data{i,j,2} = data{i,2}(b_R{i,1}(1,j):b_R{i,2}(1,j),y);
        delimitated_data{i,j,3} = data{i,2}(b_R{i,1}(1,j):b_R{i,2}(1,j),z);
    end
end
end
%% THE END