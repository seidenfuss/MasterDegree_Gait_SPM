...
for i =1: size(GRF,1)
    [discrete_GRF(i).valley1_ML_RL,discrete_GRF(i).locvalley1_ML_RL]=min(GRF{i,3}(1,1:50));
    [discrete_GRF(i).peak1_ML_RL,discrete_GRF(i).locpeak1_ML_RL]=max(GRF{i,3}(1,1:50));
    [discrete_GRF(i).peak2_ML_RL,discrete_GRF(i).locpeak2_ML_RL]=max(GRF{i,3}(1,51:101));
    discrete_GRF(i).locpeak2_ML_RL=discrete_GRF(i).locpeak2_ML_RL+50;
end


%zeros
for i = 1:size(GRF,1)
    discrete_GRF(i).inter1_ML_RL=[];
...
for i = 1:dim(1,2)
    ...
    %ML
    %total
    discrete_GRF(i).total_imp_ML_RL=trapz(abs(data(i).mean_GRF_RL{1, 3}(1,discrete_GRF(i).inter1_ML_RL:discrete_GRF(i).inter2_ML_RL)));
end
...
for d = 1:3
    ...
    for i =1:dim(1,2)
        ...
        if d==3
            ...
            hold on
            plot(discrete_GRF(i).locpeak2_ML_RL,discrete_GRF(i).peak2_ML_RL,'r*')
        end
