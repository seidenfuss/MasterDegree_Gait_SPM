function [outputArg1,outputArg2] = interSubj(data)
...
for i=1:dim(1,2)
    for d=1:3
        data(i).GRF_R{1,d}=data(i).GRF_R_corrFilter95{1, 1}{1, d};
        data(i).GRF_L{1,d}=data(i).GRF_L_corrFilter95{1, 1}{1, d};
...
plotStance2(size(data,2),data,2,7,"Mean and SD (GRF Right) - ",'Mean and SD (GRF Left) - ')
...
plotStance2(size(output_intersubj,2),output_intersubj,2,6,"Mean and SD (GRF Right) - After filter2",'Mean and SD (GRF Left) - After filter2')

%% export data: save file
save(strcat(data,'.mat'),'data')
