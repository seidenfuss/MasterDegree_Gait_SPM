...
for i=1:size(NATURE_women.adult,2)    
        YR(i,:,1)=mean(NATURE_women.adult(i).GRF_AP_R{:,4:end});
        YR(i,:,2)=mean(NATURE_women.adult(i).GRF_V_R{:,4:end});
        YR(i,:,3)=mean(NATURE_women.adult(i).GRF_ML_R{:,4:end});
        YL(i,:,1)=mean(NATURE_women.adult(i).GRF_AP_L{:,4:end});
        YL(i,:,2)=mean(NATURE_women.adult(i).GRF_V_L{:,4:end});
        YL(i,:,3)=mean(NATURE_women.adult(i).GRF_ML_L{:,4:end});
end
...
title('Hotelling T2')
xlabel("Stance Phase (%)");
sgtitle('Adult Women (15-25 yo) - GRF Walking - GaitRec Nature Database')
...
for d=1:3
    ...
    sgtitle('Young Women (15-25 yo) - GaitRec Nature Database: Post-hoc - Šidák p-value: 0.0170')
end